...
(function() {
  ...
  saveQuery = function(saveQueryButton, event) {
    ...
    if (gridProcessor = window.getGridProcessorForElement(saveQueryButton)) {
      ...
      var setAsDefault = _saveQueryButton.prev();
      inputField = _saveQueryButton.prev().prev();
      ...
      jQuery.ajax({
        ...
        async: true,
        data: requestPath + '&query_name=' + encodeURIComponent(queryName)+'&is_default='+!!$(setAsDefault).attr('checked'),
        dataType: 'json',
        success: function(data, textStatus, jqXHR) {
