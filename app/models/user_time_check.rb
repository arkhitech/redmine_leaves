class UserTimeCheck < ActiveRecord::Base
  ...
  class << self
    ...
    def email_reminder_for_issue_time_log
      cc_group = []
      reminder_time = ''
      ...
      all_users.find_each do |user|
        ...
        assigned_issues.each do |issue|
          ...
          unless time_entry
            ...
            LogTimeReminderMailer.reminder_email(user, issue, cc_group, reminder_time).deliver_now          
          elsif !(time_entry.updated_on.to_date == Date.today) && !user_leave.exists?
    ...
    def import(file, options = {:headers => false, :col_sep => "\t"} )
      unless file.nil?
